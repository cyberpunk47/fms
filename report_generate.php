...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Store report parameters in session for the download page
    $_SESSION['report_params'] = $_POST;
    
    // Redirect to report download page
...
switch ($reportType) {
    case 'faculty':
        ...
        break;
    case 'evaluation':
        $reportTitle = 'Evaluation Analysis';
        break;
    case 'department':
        $reportTitle = 'Department Analytics';
        break;
    case 'development':
        $reportTitle = 'Development & Growth Analysis';
        break;
    default:
        $reportTitle = 'Custom Report';
}

// Get departments for selection lists
try {
                ...
                <form method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
                        ...
                        <?php if ($reportType === 'evaluation'): ?>
                            ...
                            <div>
                                <label for="evaluation_department" class="block text-sm font-medium text-gray-700">Department</label>
                                <select id="evaluation_department" name="evaluation_department"
                                    class="mt-1 block w-full bg-white border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                    <option value="">All Departments</option>
                                    <?php foreach ($departments as $dept): ?>
                                    <option value="<?php echo $dept['department_id']; ?>"><?php echo htmlspecialchars($dept['name']); ?></option>
                    ...
                    <div class="mt-4">
                        <label for="include_charts" class="inline-flex items-center">
